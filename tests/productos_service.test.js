...
describe('Test : CRUD de productos_Service.js', () => {
    ...
    test('update : Debería actualizar la fecha de vencimiento a "2026-06-15" del producto con ID: 1', async () => {
        ...
        productoService.update = jest.fn().mockImplementation((idProdActualizar, camposActualizados) => {
            ...
            if (indice === -1) {
                ...
                productosMock[indice] = { ...productosMock[indice], ...camposActualizados }; // Utilizamos el operador spread para actualizar los campos del producto
                return Promise.resolve(productosMock[indice]); // Retornamos el producto actualizado como promesa resuelta
            }
        });

        const resultado = await productoService.update(idProdActualizar, camposActualizados); // Llamamos al método update desde productos_Service.js con el ID y los campos a actualizar como parámetros
    ...
    test('validarStock : Debería validar el stock de un producto ID: 5, según la cantidad requerida: 2', async () => {
        ...
        productoService.validarStock = jest.fn().mockImplementation((idProdValidar, cantidad) => {
            if (indice === -1) {
                return Promise.resolve(null); // Si no existe el producto, retornamos null
            }
            else {
        ...
        const resultado = await productoService.validarStock(idProdValidar, cantidad); // Llamamos al método validarStock desde productos_Service.js con el ID del producto y la cantidad a validar como parámetros
